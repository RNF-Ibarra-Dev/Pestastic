...
<body class="bg-official text-light">
    ...
    <div class="sa-bg container-fluid p-0 min-vh-100 d-flex">
        ...
        <main class="sa-content col-sm-10 p-0 container-fluid">
            ...
            <div class="bg-light bg-opacity-25 rounded p-3 mx-3 my-3 user-select-none">
                <h1 class="fs-1 text-center fw-bold text-light">Technician Accounts</h1>
            </div>
            ...
            <div class="table-responsive-sm d-flex justify-content-center">
                <table class="table align-middle m-3 table-hover tech-table w-100 text-light">
                                ...
                                <div class="row mb-2">
                                    <div class="col-lg-6 mb-2">
                                        <label for="fname" class="form-label fw-bold">First
                                            Name</label>
                                        <input type="text" name="fname" class="form-control-plaintext name-input" id="fname" readonly>
                                    </div>
                                    <div class="col-lg-6 mb-2">
                                        <label for="lname" class="form-label fw-bold">Last
                                            Name</label>
                                        <input type="text" name="lname" class="form-control-plaintext name-input" id="lname" readonly>
                                    </div>
                                </div>
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="col-lg-3 mb-2">
                                        <label for="emp-id" class="form-label fw-bold">Employee
                                            ID</label>
                                        <input type="number" name="empId" class="form-control-plaintext empid-input" id="emp-id" readonly>
                                        <div class="form-text d-none">
                                            e.g. 023
                                        </div>
                                    </div>
    ...
    <script>
        ...
        const dataurl = "tablecontents/tech.data.php";
        ...
        async function get_table(cur_page = 1, branch = null) {
            $.get("tablecontents/tech.table.pagination.php", {
                ostable: true,
                ...
                .fail(function (e) {
                    console.error(e);
                    $("#tech_table").html("<tr><td scope='row' colspan='5' class='text-center'>AJAX Error.</td></tr>")
                });
        }
        ...
        async function get_pagination(cur_page = 1, branch = null) {
            $.get("tablecontents/tech.table.pagination.php", {
                ...
                $("#tech_pagination").empty();
                $("#tech_pagination").append(d);
            }, 'html')
                .fail(function (e) {
                    console.error(e);
                    $("#tech_pagination").html("<p class='text-center fw-bold'>Warning: Failed at fetching pagination buttons</p>");
                });
        }
        ...
        $("#tech_pagination").on('click', '.page-link', async function (e) {
            e.preventDefault();
                ...
                delay = setTimeout(async function () {
                    ...
                    try {
                        const searchtransaction = await $.ajax({
                            url: 'tablecontents/tech.table.pagination.php',
                            type: 'GET',
                            dataType: 'html',
                            data: {
                                search: search,
                        ...
                        if (searchtransaction) {
                            if (!search == '') {
                                ...
                                $('#tech_table').append(searchtransaction);
                                $('#pagination').empty();
                            } else {
                                $('#loader').addClass('visually-hidden');
        ...
        $("#tech_table").on('click', '.edit-btn', function () {
            ...
            $.get(dataurl, {
                ...
                console.log(d);
                $("#id_input").val(d.technicianId);
                $("#fname").val(d.firstName);
                $("#lname").val(d.lastName);
                $("#usn").val(d.username);
                $("#email").val(d.techEmail);
                $("#contact-number").val(d.techContact);
                $("#address").val(d.techAddress);
                $("#emp-id").val(d.techEmpId);
                $("#display_name").text(d.firstName);

                var formattedBirthdate = new Date(d.techBirthdate);

                var bd = formattedBirthdate.toLocaleDateString('en-US', { month: "long", day: "2-digit", year: "numeric" })
                $("#bdate_info").text(bd);


                birthdate.setDate(d.techBirthdate);
